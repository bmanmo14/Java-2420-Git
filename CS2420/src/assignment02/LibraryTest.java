...
/**
 * Kale Thompson and Brandon Mouser 
 * 
 * Testing class for LibraryGeneric. This class tests the Library class for any mistakes.
 * Performs various tests to check individual functions, and prints out an error message 
...
public class LibraryTest {
	...
	public static void main(String[] args) {
		...
		if (booksCheckedOut == null
				...
				|| !booksCheckedOut.get(0).equals(
						new Book(9780330351690L, "Jon Krakauer",
								"Into the Wild"))
				|| !booksCheckedOut.get(0).getHolder().equals("Jane Doe")
				|| !booksCheckedOut.get(0).getDueDate()
		...
		if (!lib.checkout(9781843190004L, "Brandon Mouser", 1, 1, 2008))
			System.err.println("TEST FAILED -- small library: checkout");
		if (!lib.checkin(9781843190004L))
			System.err.println("TEST FAILED -- small library: checkin(isbn)");
		if (lib.checkin("Brandon Mouser"))
			System.err.println("TEST FAILED -- small library: checkin(holder)");
	...
	public static ArrayList<LibraryBook> generateLibrary(int size) {
		...
		for (int i = 0; i < size; i++) {
			...
			result.add(new LibraryBook(Long.parseLong(isbn), "An author",
					"A title"));
		}
